import google_trends from './img/google_trends.png';
import lorem_ipsum from './img/lorem_ipsum.png';
import lorem_picsum from './img/lorem_picsum.png';
import apibank from './img/apibank.png';
import qr from './img/qr.png';
import tameshigaki from './img/tameshigaki.png';
import sute_mail from './img/sute_mail.png';
import remove_bg from './img/remove_bg.png';
import renderforest from './img/renderforest.png';
import visualsitemaps from './img/visualsitemaps.png';
import slides_code_highlighter from './img/slides_code_highlighter.png';
import virustotal from './img/virustotal.png';
import codic from './img/codic.png';
import feedly from './img/feedly.png';
import see_ss from './img/see_ss.png';
import ux_mov from './img/ux_mov.png';

export const usefulSites = [
    {
        title: 'Google Trend',
        url: 'https://trends.google.co.jp/trends/?geo=JP',
        image: google_trends,
        description:
            'ある単語がGoogleでどれだけ検索されているかというトレンドをグラフで見ることができるツール。',
    },
    {
        title: 'Lorem Ipsum',
        url: 'https://www.lipsum.com/',
        image: lorem_ipsum,
        description:
            '出版、ウェブデザイン、グラフィックデザインなどの諸分野において使用されている典型的なダミーテキスト。',
    },
    {
        title: 'Lorem Picsum',
        url: 'https://picsum.photos/',
        image: lorem_picsum,
        description:
            'フォトサービス「Unsplash」の画像を利用したWEBサービスで、無料のダミー画像を使用できる。',
    },
    {
        title: 'API Bank',
        url: 'https://www.apibank.jp/ApiBank/main',
        image: apibank,
        description:
            'APIを使って欲しい企業・提供者・ソフト会社とAPI開発者・ビジネス企画者を繋げる流通プラットフォーム。',
    },
    {
        title: 'QRのススメ',
        url: 'https://qr.quel.jp/',
        image: qr,
        description: 'メールアドレス、任意のテキストなどのQRコードを作成できる Web サービス。',
    },
    {
        title: 'ためしがき',
        url: 'https://tameshigaki.jp/',
        image: tameshigaki,
        description:
            '商用利用可能な日本語のフリーフォントをお好きな言葉・文章で試すことができるWebサイト。',
    },
    {
        title: '捨てメアド',
        url: 'https://m.kuku.lu/',
        image: sute_mail,
        description:
            '多機能フリーメールサービス。メールアドレスは同時にいくつでも保有でき、期限なし。登録も不要。',
    },
    {
        title: 'remove.bg',
        url: 'https://www.remove.bg/ja',
        image: remove_bg,
        description: '人物や商品などの写真から背景だけを取り除くツール。',
    },
    {
        title: 'RenderForest',
        url: 'https://www.renderforest.com/jp/#Videos',
        image: renderforest,
        description:
            'ロゴのデザインからプロ並みの動画、モックアップ、画像、ホームページまで作成できるオールインワンのツール。',
    },
    {
        title: 'VisualSiteMaps',
        url: 'https://visualsitemaps.com/',
        image: visualsitemaps,
        description: '指定したサイトのサイトマップ情報を視覚化してくれるオンラインサービス。',
    },
    {
        title: 'SlidesCodeHighlighter',
        url: 'https://romannurik.github.io/SlidesCodeHighlighter/',
        image: slides_code_highlighter,
        description:
            'ソースコードにシンタックスハイライトで色付けしてくれるWebツール。カラフルなコードにしてパワポなどに貼ろう。',
    },
    {
        title: 'VirusTotal',
        url: 'https://www.virustotal.com/gui/home/url',
        image: virustotal,
        description:
            'ファイルやウェブサイトのマルウェア検査を行うウェブサイト。対象のファイルやウェブサイトが「マルウェアを含むかどうか」検査できる。',
    },
    {
        title: 'codic',
        url: 'https://codic.jp/',
        image: codic,
        description:
            '日本語を入力すると変数名を提示してくれるサービス。命名作業は結構頭持ってかれるので嬉しい。',
    },
    {
        title: 'Feedly',
        url: 'https://feedly.com/i/welcome/logged-out',
        image: feedly,
        description:
            '登録したサイトの更新を監視できるRSSリーダー。PCをメインで使用する者にとってRSSリーダーはまだオワコンではない。',
    },
    {
        title: 'See-SS',
        url: 'https://see-ss.com/',
        image: see_ss,
        description: '見やすさにこだわっているHTML/CSSのスニペットサイト。一覧でかなり見やすい。',
    },
    {
        title: 'UX-mov',
        url: 'https://ux-mov.com/',
        image: ux_mov,
        description: 'アプリのUIが動画で見れるギャラリーサイト。UIデザインの参考になる。',
    },
];
